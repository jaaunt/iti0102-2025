...
def is_name_in_password(password: str, name: str) -> bool:
    ...
    if (name1 in password or name2 in password or name3 in password
            or rev_name1 in password or rev_name2 in password or rev_name3 in password):
        return True
    return False
...
def is_password_valid(new_password: str, old_password: str, name: str, birthdate: str) -> bool:
    ...
    """
    return (is_correct_length(new_password)
            and includes_uppercase(new_password)
            and includes_lowercase(new_password)
            and includes_number(new_password)
            and includes_special(new_password)
            and is_different_from_old_password(old_password, new_password)
            and not is_name_in_password(new_password, name)
            and not is_birthday_in_password(new_password, birthdate))  # annab true ainult siis kui koik on vaartusega true
